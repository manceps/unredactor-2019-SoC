...
Let's specify some global variables to hold things like paths. Globals in python are always written in all caps. That makes it easier to find them and replace them with local variables later. Local variables make your code more modular, easier to reuse and maintain.
"""
...
These are functions we used to process the Mueller report. You may not ever need them. But there here in case you wan to learn more about regular expressions.

You can use this function to find redactions surrounded by square brackets, if you like. But we'll just use the "unk" marker for now. So you don't even have to run this cell.
"""

def find_redactions(s):
    return re.findall(r'\[[^\]]*\]', s)

"""We used this function to try to find recactions markers in the Mueller Report. The OCR usually contained redactions marked with "[HARM]", or "[Personal Privacy]" or "[Grand Jury]", so this function returns a list of some bits of text that look like that."""
...
unredacted_text = unredact("To be or not to unk, that is the question.")

print(unredacted_text)

unredacted_text = unredact("To be or not to unk, that is the question.", redacted_tokens=['be'])

print(unredacted_text)

unredacted_text = unredact("Is the president of the United States named unk unk ?", redacted_tokens=['Barak', 'Obama'])

unredacted_text = unredact("And now I'd like to introduce you to the 2008 President of the United States of America, unk unk !", redacted_tokens=['Barak', 'Obama'])

"""## Cased BERT Model

So the uncased BERT model did fine on sentences where case isn't important. But let's see if it can do better on proper nouns (famous people names). We'll need to load the "cased" BERT model for that.
"""
...
edacted_text = unredact("The President of the United States of America after George W Bush was unk unk .", redacted_tokens=['Barak', 'Obama'])

unredact("The President of the United States, unk , made an announcement today about the Iraq war.", redacted_tokens=['Bush'])
